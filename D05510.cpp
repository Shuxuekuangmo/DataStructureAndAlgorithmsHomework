...
{
	MGraph G;
	char filename[] = "GData.txt";
	CreatGraph(G, filename);	showGraph(G);

	InsertVec(G, 'z');	showGraph(G);
	InsertArc(G, 'z', 'a');	showGraph(G);
	DleeteArc(G, 'z', 'a');	showGraph(G);
	DleeteVex(G, 'z');	showGraph(G);
	DleeteVex(G, 'a');	showGraph(G);
	return 0;
}