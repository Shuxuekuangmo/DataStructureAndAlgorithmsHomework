...
void BiTreeShowNotLessThanX(BiTree BT, KeyType x, int compare(KeyType a, KeyType b))
{
	if (BT == NULL)return;
	if (compare(BT->data.key, x) < 0)BiTreeShowNotLessThanX(BT->right, x, compare);
	else
	{
		BiTreeShowNotLessThanX(BT->right, x, compare);
		cout << BT->data.key << " ";
		BiTreeShowNotLessThanX(BT->left, x, compare);
	}
...
int main()
{
	...
	BiTreeLists(BT, show);
	cout << endl;
	BiTreeShowNotLessThanX(BT, 4, compare);
	return 0;
}